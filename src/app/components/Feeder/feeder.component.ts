...
import {   areaFeederModel, FeederInfoModel } from 'src/assets/data';
...
export class FeederComponent {
  ...
  ruevenTable = new areaFeederModel();
  lensTable = new areaFeederModel();
  roodeportTable =  new areaFeederModel();
  hurstHillTable = new areaFeederModel();
 northTable =  new areaFeederModel();
 revTempModel:FeederInfoModel[] = [];
 lensTempModel:FeederInfoModel[] = [];
 hurlTempModel:FeederInfoModel[] = [];
 roodTempModel:FeederInfoModel[] = [];
 northTempModel:FeederInfoModel[] = [];
...
this.feederData.forEach(result => {
  if(result[0] == "Rueven")
    {
      alert("Rueven");
    }
...
var currentheader = "";
  ...
  }else if(arr.length > 2 && arr.length > 0)
  {
    this.newTqable = true;
      ...
      let temp:FeederInfoModel= {
      
        sdc: result[0],
        substation: result[1],
        feederDesc: result[2],
        BreakerType: result[3],
